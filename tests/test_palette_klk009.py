# KLK-009 複数案(最大3案)一括生成＋SCR-002 比較・閲覧画面のテストを
# unittest スイートへ束ねるラッパー（tester所有）。
# - S群（静的）: tests/site/check_klk009.py（S1-S15・Python標準のみ・
#   対象＝ゴールデン fixtures/klk009/{compare,compare-partial,index-a/b/c}.html
#   ＋ instruction.json / DRAFT_RULES.md / SKILL.md）
# - D群（動的）: git check-ignore サブプロセスで mockups/ 複数案の Git 除外成立を
#   検証（D1・git不在時skip）。
#
# M群（/draft-generate 実生成＋ブラウザ実機）は自動化不能のため tester が手動確認し
# チケットのログへ記録する（check_klk007/008 ラッパーと同型）。
import shutil
import subprocess
import unittest
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent
STATIC_CHECKER = ROOT / "tests" / "site" / "check_klk009.py"


class TestKLK009Static(unittest.TestCase):
    """check_klk009.py（設計書 §9 S群 S1-S15）が全PASSすること。"""

    def test_static_checks_pass(self):
        proc = subprocess.run(
            ["python3", str(STATIC_CHECKER)],
            capture_output=True, text=True, cwd=str(ROOT), timeout=120,
        )
        self.assertEqual(
            proc.returncode, 0,
            "check_klk009.py failed:\n" + proc.stdout + proc.stderr,
        )


@unittest.skipUnless(shutil.which("git"), "git が見つからないため check-ignore をskip")
class TestKLK009GitIgnore(unittest.TestCase):
    """D1: mockups/{日付}_{案件名}/ 配下の複数案生成物（compare.html /
    index-a.html / instruction.json）が .gitignore で除外され
    git check-ignore が exit 0（除外成立）を返すこと。（REQ-011 / NFR-004）"""

    TARGETS = [
        "mockups/2026-07-08_サンプル案件/compare.html",
        "mockups/2026-07-08_サンプル案件/index-a.html",
        "mockups/2026-07-08_サンプル案件/instruction.json",
    ]

    def _check_ignored(self, path):
        # git check-ignore: exit 0 = 除外成立, 1 = 未除外, 128 = git管理外/エラー
        return subprocess.run(
            ["git", "check-ignore", path],
            capture_output=True, text=True, cwd=str(ROOT), timeout=60,
        )

    def test_d1_mockups_ignored(self):
        # git 管理下でない環境は skip（fail にしない）
        pre = subprocess.run(
            ["git", "rev-parse", "--is-inside-work-tree"],
            capture_output=True, text=True, cwd=str(ROOT), timeout=60,
        )
        if pre.returncode != 0:
            self.skipTest("git リポジトリ外のため D1 をskip")
        for target in self.TARGETS:
            with self.subTest(path=target):
                proc = self._check_ignored(target)
                self.assertEqual(
                    proc.returncode, 0,
                    f"git check-ignore で除外不成立(exit {proc.returncode}): {target}\n"
                    f"{proc.stdout}{proc.stderr}",
                )


if __name__ == "__main__":
    unittest.main()
