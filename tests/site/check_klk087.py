...
# D群 — 画面の説明が実態と合っているか
# ===========================================================================
# ★契約更新（KLK-088 で生成対応が済んだため）:
#   KLK-087 の時点では「まだ生成には反映されません」と正直に書くことを要求していた。
#   088 で反映されたので、要求は「**何ができるかが正確に書いてある**」へ変わる。
#   注記を消したとき D1/D2 が意図的に FAIL し、それに気づいてここを更新した
#   （KLK-061 と同じ消し忘れ防止装置は、2度目も正しく作動した）。
check(
    "D1 実装中の「まだ反映されません」注記が残っていない（KLK-088 で反映済み）",
    "まだ生成には反映されません" not in INDEX and 'id="compPendingNote"' not in INDEX,
    "旧注記の残存=%s" % ("まだ生成には反映されません" in INDEX),
)
check(
    "D2 画面が composition の効き方を正確に説明している（並びは3案共通・型指定は全案同一）",
    'id="compNote"' in INDEX
    and "並び順は3案とも同じ" in INDEX
    and "3案ともその型" in INDEX
    and "同じセクションを複数置けます" in INDEX,
    "注記=%s / 並び全案共通=%s / 型指定=%s / 複数配置=%s"
    % ('id="compNote"' in INDEX, "並び順は3案とも同じ" in INDEX,
       "3案ともその型" in INDEX, "同じセクションを複数置けます" in INDEX),
)
check(
    "D2b 画面がドラッグでも並べ替えられることを書いている（理恵さんがマニュアルにも要望）",
    "⠿ を掴んで上下にドラッグ" in INDEX,
    "ドラッグの案内=%s" % ("⠿ を掴んで上下にドラッグ" in INDEX),
)
