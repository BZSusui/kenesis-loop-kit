...
- **① ジョブ仕様 `.json` パス(ローカルブリッジ経由)**: `mockups/.pending/{jobId}.regen.json`
  (`{schema:"design-regenerate-job", version:1, target, addr, desiredType?}`)。引数がこのファイルパスのときは**そのファイルを読み込み**、
  ...
  - **`desiredType`(任意・KLK-079)**: あるときは**その型で作る**(手順3の最優先)。無いときは従来どおり。
    ブリッジが**番地ごとのプールに載っている語彙だけ**を書くので、値の妥当性を疑う必要はない。
- **② 手動 `$ARGUMENTS`**:
  ...
  - **型を指定する場合は末尾に型マーカーを足す**(KLK-079・例 `mockups/2026-07-08_サンプル案件 a GALLERY-01 pat-masonry`)。
    §12.1.2/§12.1.3 のその番地のプールに無い語なら、**ファイルを変更せず停止**して選べる型を案内する。
...
- **★型指定の最優先(KLK-079・`desiredType`)**: ジョブ仕様に `desiredType` があるときは、
  **下の表引きも参考準拠の現行マーカーも見ず、指定された型で作る**。
- **★`composition` の型指定(KLK-088・`desiredType` の次に強い)**: `desiredType` が無く、
  併置 `instruction.json` に **`composition`** があるときは、対象番地の**出現順 k**（`MENU-02` なら k=2）から
  **同じ KEY の k 番目のエントリ**を引き、その `type` があれば**それで作る**（表引き・参考準拠より優先）。
  `type` が無ければ、表引きの結果に **`(k-1)` を足して `mod 6`**（インスタンス補正・DRAFT_RULES §12.1.2(3.1)）。
  そのエントリの `heading`/`lead`/`moreLink` も**そのインスタンスの指定として反映**する。
  - 容器 `.m-{sec}`(HERO は `.m-hero` の `data-hero`)に**そのマーカーだけ**を付ける。
    **元の型マーカーは必ず外す**(2つ付いたままにしない＝CSS が競合して崩れる)。
  - その型の CSS が `<head>` に無ければ**足す**。使わなくなった旧型の CSS は残してよい(他案と共有のため)。
  - 型の見た目は §12.1.2 / §12.1.3 の**その型の定義どおり**に作る。§3.0(アタリ 4/3)・§8.1(狭いカラムでの畳み方)
    などの**横断ルールは型定義より優先**する(§3.0/§8.1 の明記どおり)。
  - **確認**: 書き戻したあと、対象 `.sec` に `desiredType` のマーカーが**ちょうど1つ**あり、
    同じプールの他のマーカーが**残っていない**ことを自分で確かめてから完了を報告する。
    ブリッジも同じことを実ファイルで検証し、食い違えば `typeApplied:false` として利用者に伝える
    (＝**黙って成功と言えない**)。
- **プールマーカー再付与(KLK-029/036/037・DRAFT_RULES §12.1.2/§12.1.3/§14)**: **`desiredType` が無いとき**に適用する。 対象が `VOICE-01`/`FLOW-01`/`STAFF-01`(§12.1.2・mod6)
