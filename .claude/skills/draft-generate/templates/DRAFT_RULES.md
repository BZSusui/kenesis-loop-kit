...
#### 2.0.1 同じセクションを複数置くときの採番（KLK-088・`composition`）

`instruction.composition`（§2.2）があるとき、**同じ KEY のインスタンスには出現順に `-01` `-02` `-03`** を振る。

| composition の並び | 番地 |
|---|---|
| ABOUT, MENU, CTA, MENU, GALLERY, CTA | `ABOUT-01` `MENU-01` `CTA-01` `MENU-02` `GALLERY-01` `CTA-02` |

- **1個目は必ず `-01`**。1個しか無いセクションは従来と同じ番地になる＝**既存生成物・見本・golden と完全互換**。
- 番地は**ページ内で一意**（§14 の部分再生成が「ちょうど1回」を前提にしているため。ここが崩れると
  🔄 が「番地が重複しています」で停止する）。
- `NAV-01` / `MV-01` / `FOOTER-01` は composition の対象外＝常に1つ・固定位置（先頭/最後）。
...
- **並び順**: `composition` が**無い**ときは上の canonical 順のうち**選ばれた分だけ**（従来どおり）。
  案ごとの並び替え・型振りは KLK-023。**`composition` があるときは §2.2 が並び順の正**。
...
### 2.2 ページ構成 `composition`（KLK-086/087/088・同一セクションの複数配置・並び順・個別設定）

実案件の原稿には「MENU が『ランチ』『ディナー』で2回出る」「原稿どおりの並び」がある。
`sections`（重複なし・canonical 順）では表せないので、**順序つき・重複あり・個別設定つき**の
本文構成を additive に足す。
...
"composition": [
  ...
  { "key": "CTA" },
  { "key": "MENU",  "heading": "ディナー", "type": "price-table",
    "moreLink": { "label": "メニュー一覧へ", "href": "/menu/" } },
  { "key": "GALLERY" }
]
```
...
1. **`composition` があれば、それが本文構成の正**。配列の順＝ページの並び（**全案共通**）。
   番地は §2.0.1 の採番（出現順に `-01` `-02` `-03`）。
2. **無ければ従来どおり** `sections` を canonical 順で出す（**既存の指示書は1バイトも解釈が変わらない**）。
3. エントリの個別設定は §4.2（`heading`/`lead`）・§4.3（`moreLink`）・型（§12.1.x）と**同じ意味**。
   そのインスタンスにだけ効く。
4. `sections` は `composition` のキー集合（canonical 順・重複除去）と**一致していなければならない**。
   食い違う指示書は**受付チェックで停止**する（曖昧なまま長い生成を走らせない）。
5. `sectionOptions.{KEY}.heading/lead` も併記されることがあるが、
   **`composition` があるときはエントリ側が優先**（併記は composition を知らない読み手への保険）。
   `sectionOptions.CTA.purpose/label` は**全 CTA インスタンス共通**（ボタン構成は共有）。

**置ける数（KLK-086・理恵さん決定）:**

| 区分 | 上限 |
|---|---|
| 複製可（各3個まで） | ABOUT / MENU / GALLERY / VOICE / FLOW / STAFF / NEWS / PRICE / FAQ / SNS / **CTA** |
| 1個のみ | ACCESS / CONTACT / SEARCH |
| 本文合計 | **12個** |

- **CTA を複製できる**のは「同一内容をページ上部と下部に置く」用途のため。
- **SEARCH** は §12.1.3(7) で HERO/NAV へ埋め込むので**独立セクションを出さない**＝
  並び順の影響を受けない（composition に入っていても位置は関係しない）。
...
#### 3.0.2 実際に描いたときの許容帯（KLK-118）

CSS に `aspect-ratio: 4/3` と書いてあっても、**描かれた結果が細長い**ことがある
（狭いカラムに入る・`grid` のセルに引き伸ばされる・`span` で帯になる）。
宣言ではなく**実測**で次の帯に収める。`tools/verify-responsive.node.js` が実描画で検査する。

| 対象 | 実測比率（幅 ÷ 高さ）の許容 | 根拠 |
|---|---|---|
| 通常のアタリ | **0.68 〜 1.47** | 縦 3:4（0.75）〜 横 4:3（1.333）に1割の余裕。`1/1` もこの中 |
| 混在タイル型（`pat-mosaic` / `pat-masonry` / `sns-masonry`） | **0.40 〜 3.00** | 設計上の想定が 1×2≒0.5 / 2×2≒1.333 / 2×1≒2.67。**個々のタイルは問わず上限だけ**見る |
| `panel-band` のフィルムのコマ | **1.35 〜 1.65** | `3/2`（KLK-043） |
| HERO の全面ビジュアル | 対象外 | 画面を覆うもの（§4.3.3） |

- **`16/9`（1.778）は通らない。** 「横長」「ワイド」と書かれた型でも、この帯の外に出してはならない
- 混在タイル型で **1つのタイルを1行の高さのまま3列以上にまたがせない**（帯になる）。
  広く見せたいときは行もまたぐ（`grid-row: span 2`）。
  実例: `.atari:nth-child(6){grid-column:span 4}` ＋ `grid-auto-rows:140px` で
  **720×140＝5.143** の帯が生成された（KLK-118・2026-09-05 の生成物）
- 数値は116ファイル・全84型の実測から決めた（KLK-118）

**なぜ 4/3 か**: 実績カタログの参考画像も一般的な写真も横長が多く、`16/9` ほど細くない
`4/3` が「写真らしさ」と「縦の情報量」の折り合いが良い。正方に近づくため、
サイドバーのような狭い幅でも帯にならない。

#### 3.0.3 列数の多い表は、幅に関係なく横スクロールできる器に入れる（KLK-118）

`map-hours` の診療時間表・`news-table`・`price-table`・`price-matrix` のように
**列が多い表**は、狭い幅で内容幅が箱を超え、**ページ全体に横スクロールを出す**。

- これらの表は **`overflow-x: auto` の器で包む**（`<div class="table-wrap">` 等）。
  **「モバイルは横スクロール」と書いてあっても、640px 以下に限定しない。**
  実際に破綻するのは **641〜900px**（PC レイアウトのまま幅が狭まる帯）である
- 表そのものに `overflow-x` を付けない（`<table>` の表示が崩れる）。**包む**こと
- 実例: 見本03（クリニック）の `map-hours` が 768px で `scrollWidth 800 > 768` となり、
  背景からも画面からもはみ出していた（KLK-118）
...
#### 4.1.2 生成物に開発側の記号を書かない（KLK-085）

生成したページに**要件ID・チケットID・画面ID を文字として出さない**。
`REQ-010` `KLK-022` `SCR-001` `NFR-005` `OQ-003` のような記号は、**利用者には何の番号か分からない**。

- 禁止するのは**画面に見える文字**。HTML コメント（`<!-- SCR-001 -->`）は残してよい（追跡に要る）。
- **とくに間違えやすいのが `compare.html` のボタン名**。実際に
  `← 設定を変えて再生成（SCR-001）` `← 設定を変えるには SCR-001 で作り直してください`
  という文字列が生成された（見本01・02）。**画面の名前は「設定画面」と書く**。

| 書いてはいけない | こう書く |
|---|---|
| `← 設定を変えて再生成（SCR-001）` | `← 設定を変えて再生成` |
| `SCR-001 で作り直してください` | `設定画面で作り直してください` |
| `（REQ-102）` のような括弧書き | 書かない。消しても意味が通る文にする |

**なぜ**: 生成物は社外の方の目にも触れる（提案資料として印刷・PDF化される）。
社内の管理記号がそこに出ていると、受け取った側には意味不明なノイズにしかならない。
番地ラベル（`MV-01` 等）は**別物**で、これは残す（🔄 セクション再生成で使う名札）。
...
### 8.2 短いラベルを折らない（KLK-122）

ナビ項目・ステップ見出し・ボタン文言のような**短いラベル**（おおむね8文字以内）を、
**1行に6文字も入らない幅へ詰めてはならない**。「アクセス」が「アクセ／ス」と折れる。

実際に起きた例（見本03・768px）:

| 要素 | 実測 | 症状 |
|---|---|---|
| NAV の項目 | 幅 40〜54px / 14px ＝ 1行 2.9〜4 文字 | 「アクセ／ス」「医院紹／介」 |
| FLOW の見出し | 幅 69px / 13.5px ＝ 1行 5.1 文字 | 「メンテナン／ス」 |

**1280px では1件も起きない。** PC レイアウトのまま幅が狭まる **641〜900px** 特有である。

- **直す順番**: ①器の余白・項目間隔を詰める ②列数や折り返し方を変える
  （`flex-wrap:wrap` 等）③それでも入らないときだけ文字を小さくする。
  **ラベル自体を折る前提で組まない**
- `justify-content:space-between` のまま項目を潰さない。潰れるのは
  flex/grid の子が既定で中身より小さくなれるため
- 検査: `tools/verify-responsive.node.js` が実描画で見る（判定 F）。
  ★**箱の高さ ÷ 行高では測れない**（ボタンの上下余白で2行に見える）。
  `Range` で文字の行box を数えている。
  なお判定は「1文字＝1em」の近似なので、**英字のラベルは見落とす側に外れる**（取りこぼしは出るが誤検出は出ない）

### 8.3 見出しと説明文を同じ行に流さない（KLK-122）

セクション内の小見出し（`.t` 等）と、その下の説明文（`.d` 等）は**行を分ける**。

`<span>` のままだと inline なので、`margin` を指定しても**同じ行につながる**。
実例（見本03 の FLOW・幅に関係なく発生していた）:

```
✗ ご予約お電話・WEBから        ← 見出しと説明が地続きに見える
✓ ご予約
  お電話・WEBから
```

- 見出し・説明の要素には `display:block` を与える（`margin` だけでは効かない）
- **幅を狭めると目立つが、原因は幅ではない。** 1280px でも同じように起きていた
...
**★`composition` があるときの条件つき改訂（KLK-088）:**

| 不変条件 | `composition` 無し | `composition` あり |
|---|---|---|
| ① `data-columns` 全案同一 | 維持 | 維持 |
| ② セクション集合 全案同一 | 維持 | 維持（**多重集合**として同一＝MENU が2つなら3案とも2つ） |
| ③ `--m-main` 相違 ④ `data-archetype` 相違 | 維持 | 維持 |
| **⑤ `data-section-order` 案間相違** | 維持 | **全案同一へ**（並びは人が指定したもの・理恵さん決定1） |
| **⑥〜⑨ 型の案間相違** | 維持 | **`type` を明示していない番地についてのみ**要求 |

- **なぜ⑤を反転させるか**: 原稿がある案件では並びは決まっている。そこを案ごとに変えると
  「原稿どおりに作った」と言えなくなる。案の違いは**配色とレイアウト型**が担う。
- `data-section-order` は**番地**を並べる（`ABOUT-01,MENU-01,CTA-01,MENU-02,…`）。
  KEY だけだと同じセクションの2つ目を表せないため。`composition` 無しのときは従来どおり。
- **型を明示した番地が3案で同じ型になるのは意図どおり**（理恵さんが「その型で」と指定したもの）。**⑥HERO型・⑧GALLERY型・⑨ABOUT配置は KLK-036/037 以降 §12.1.3 の表引きで決定**
...
**(3.1) インスタンス補正（KLK-088・同じセクションが複数あるとき）:**

`composition`（§2.2）で同じ KEY が k 個あるとき、**k 番目のインスタンス**の pool index は
表引きの結果から **`(idx + (k-1)) mod 6`** へずらす（k は 1 始まり）。

| インスタンス | 案A | 案B | 案C |
|---|---|---|---|
| 1個目（`-01`） | `idx_A` | `idx_B` | `idx_C` |
| 2個目（`-02`） | `(idx_A+1) mod 6` | `(idx_B+1) mod 6` | `(idx_C+1) mod 6` |
| 3個目（`-03`） | `(idx_A+2) mod 6` | `(idx_B+2) mod 6` | `(idx_C+2) mod 6` |

- **なぜずらすか**: 同じページに同じ型のセクションが2つ並ぶと、複数置いた意味が薄れる
  （理恵さん決定4「先のインスタンスと同じ型を避けて自動で振る」）。
- **3案とも同じ幅でずらす**ので、**案間の distinct（§12.1 不変条件⑥〜⑨）は保たれる**。
- **型を明示したインスタンス（`composition[i].type`）は補正の対象外**。指定どおりの型を全案で使う。
  明示した型と補正結果がぶつかったときは、**補正側を +1 ずつ送って重複を避ける**（決定的）。
...
| 2 | `pat-mosaic` | 大小モザイク（`grid`＋`grid-column/row span` 強弱）。**1つのタイルを1行の高さのまま3列以上にまたがせない**（`grid-column:span 4` ＋ `grid-auto-rows:140px` で 720×140＝比率 5.14 の帯が生成された・KLK-118）。広く見せるときは `grid-row` も足す。実測比率の許容は §3.0.2（0.40〜3.00）。モバイル2列 |
...
| 3 | `img-overlap`（KLK-037新・KLK-038調整・KLK-129幅） | せり出し横長画像＋白背景文言の重なり（画像を左、白背景文言を右から `transform` で重ね）。**★列は `display:grid;grid-template-columns:minmax(0,1fr) minmax(0,480px)`・全体 `max-width:1060px` 程度。`1fr 1fr` と書いてはならない**（下記 §12.1.3.1）。**画像は文言背景に対し縦に余裕をとる（`min-height` を大きめ・上下に余白＝圧迫感を避けるモダン志向）**。モバイルは重なり解除・縦積み |
...
##### 12.1.3.1 `img-overlap` の列は `1fr 1fr` で書かない（KLK-129）

`1fr` は `minmax(auto, 1fr)` であり、**中身の最小幅より小さくならない**。
画像側のアタリには「検索: children clinic interior」のような**折り返さない一行**が入るので、
`1fr 1fr` と書くと画像側がその一行に押し広げられ、**文言側だけが削られる**。

実測（理恵さんのテストサイト・ビューポート1280px・`max-width:860px`）:

| | `1fr 1fr` と書いたとき | あるべき姿 |
|---|---|---|
| 画像側 | 587px | 半分（430px） |
| 白背景の文言側 | **273px** | 半分（430px） |

273px では見出し「子どもの健やかな毎日を、地域で支える」が2行に割れ、
本文も数文字ごとに折り返して読めない。実使用で指摘を受けた（2026-09-15）。

**書き方（4案を目視で比べて理恵さんが選んだ幅）:**

```css
.m-about.img-overlap{
  display: grid;
  /* ★文言側を先に確保し、画像は残りを取る。minmax(0,…) が要点 */
...
**なぜ「割合」ではなく「文言側を固定」なのか。** 割合（例 `55% 45%`）にすると、
器の広い案件では画像まで一緒に大きくなる。実測すると見本03で画像が 450→550px に太った。
理恵さんの指定は「**画像はそのまま・文言側を広げる**」なので、
広げたい側を先に決め、画像に残りを渡すほうが指定どおりになる。

実測（同じ書き方で2ページ）:

| | 画像 | 白背景 |
|---|---|---|
| 理恵さんのテストサイト（前） | 587px | 273px |
| 同（後） | **587px（変わらない）** | **480px** |
| 見本03（前） | 450px | 450px |
| 同（後） | 520px | **480px** |

- ★`minmax(0, ...)` を外すと同じ不具合が戻る。**`1fr 1fr` も `55% 45%` 単独も不可**
  （素の `%` トラックも最小は auto なので中身に押し広げられる）
- 文言側は **460px 以上**を確保する

**★横並びは 900px までにする（実測で必要と分かった）。**
文言側に 480px を固定すると、器が 900px を切ったとき画像側の取り分が中身より狭くなる。
768px で実測したら**画像側 208px に対し中身 400px**で、KLK-118 の実描画検査が
「中身が箱からあふれる」を検出した。よって次を必ず書く。

```css
@media (max-width: 900px){
  .m-about.img-overlap{ grid-template-columns: 1fr; }
  /* ★1列にするだけでは畳めない。子が grid-column:1/2・2/3 で自分の列を指しており、
     暗黙の2列目が作られて横並びのまま残る（実測）。grid-column も戻すこと。 */
  .m-about.img-overlap .atari,
  .m-about.img-overlap .txt{ grid-column: 1; grid-row: auto; }
  .m-about.img-overlap .txt{ transform: none; margin-top: -40px; }
}
```

**★画像側には `min-width: 0` を付ける。**
グリッドの子は既定で中身より小さくならない（`min-width:auto`）。
アタリの「検索: …」の一行が箱の幅を保ってしまい、狭い画面ではみ出す。

**★縦積みにしたら `min-height` を外す（これが 375px はみ出しの正体）。**
この型のアタリは「縦に余裕をとる」ため `min-height` を大きめに、かつ `aspect-ratio:4/3`
を併せて持つ。すると**幅が `min-height × 4/3` より小さくならない**。

| 見本 | `min-height` | そこから決まる最小の幅 | 375px 画面で |
|---|---|---|---|
| 見本03 | 300px | **400px** | はみ出す |
| 実案件の例 | 440px | **587px** | 大きくはみ出す |

縦積みの側（`@media (max-width:900px)` の中）に必ず書く:

```css
.m-about.img-overlap .atari{ min-height: 0; min-width: 0; }
```

★これを入れると、ページ全体の横スクロールも一緒に消える。
375px で「幅のある表のせいでページが横に伸びている」ように見えても、
**実際の原因はこのアタリだったことがある**（実測で確認）。表を疑う前にここを見ること。
なお**幅のある表そのものは縮めない**。器（`overflow-x:auto`）の中で横スクロールさせる（§3.0.3）。

**同じ注意が要る型**: 左右に分ける型で片側にアタリの検索キーワード行が入るもの
（`img-left` / `img-right` / `img-circle` / `feature-large` / `map-side` / `staff-feature` など）。
片側が痩せていないか、生成後に §8 の実測で確かめること。
...
**★`composition` があるとき（KLK-088）**: 参考準拠が効くのは各 KEY の**第1インスタンス（`-01`）だけ**。
2つ目以降は参考の型を借りない（参考サイトは「そのセクションを1つ」持っているだけなので、
2つ目に同じ型を写すと同じ見た目が並ぶ）。2つ目以降は §12.1.2(3.1) のインスタンス補正で振る。
...
### ★13.0 compare.html は書かない。道具で書き出す（KLK-103・最重要）

**生成側は `compare.html` を書いてはならない。** 案の HTML（`index-a/b/c.html` または
`index.html`）を書き終えたら、次のコマンドを実行するだけでよい。
...
案の数（1 or 3）は**フォルダの中身から自動判別**する。ローカルブリッジ経由の生成では
**ブリッジが自動で実行する**ので、その場合は何もしなくてよい。

**★なぜ書かせないのか（KLK-102 の実害）**
`compare.html` の JS（約160行）は**デザインではなく道具立て**で、どの生成物でも中身は同じ。
にもかかわらず毎回書き起こさせていたため、KLK-079/080/081/092 で積み上げた振る舞いが
**作り直しのたびに落ちた**。2026-09-07 に見本3点を作り直したとき、6項目中**4項目が欠落**した:

| 落ちた振る舞い | 出典 |
|---|---|
| 型セレクタ本体 `<select id="regen-type">` | KLK-079 |
| 現在と違う型のときだけ `desiredType` を送る | KLK-079 |
| `typeApplied === false` を成功と同じ見た目にしない | KLK-079 |
| 見本ガード（`mockups/` の外で固まらせない） | KLK-081 |

**★教訓: 毎回同じものを書かせるな。** 可変でないものを生成対象に置くと、
規約をいくら詳しく書いても細部は落ちる。**固定できるものは固定する。**

以下 13.1 以降は、**テンプレートが満たしている契約**の記録である（生成側の手引きではなく、
テンプレートを直すときの仕様書として読むこと）。

### 13.1 構造の契約

**`output.variants` の値によらず**、`compare.html`（＝**単一ファイル・外部依存ゼロ**）を
`mockups/{…}/` に置く（道具が案の数を自動判別して組む）。
見た目の正は `docs/wireframes/SCR-002-compare.html` の chrome。技術制約に沿って作り直す
（ワイヤーを本番へコピーしない）。

**案件ごとに変わるのは配色4色だけ**で、`:root` の CSS 変数
（`--c-main` / `--c-sub` / `--c-accent` / `--c-bg`）として差し込む。chrome 自身の色は固定。
...
- **型 `<select id="regen-type">`（KLK-079）**: `GET /sections` が返す `pool` で組み立てる。
  **選択肢の表示は `日本語ラベル（マーカー）` の形にする（KLK-117）**。英語のマーカーだけでは
  どんな型かが掴めない、という実際に使った人の指摘による。ラベルは `GET /sections` の `labels`
  （ブリッジが持つ `SECTION_TYPE_LABELS`）から取り、**送る `desiredType` はマーカーのまま**。
  `labels` を返さないブリッジではマーカーだけを出す（壊れない）。
  **ラベルの文言は本規約 §12.1.2／§12.1.3 のその型の説明文からの抜き書きとする**
  （言い換えて作らない。乖離は `check_klk117` が語ごとに照合して落とす）。
...
2.5. **`composition` があるとき（KLK-088）**: 対象番地の**出現順 k**（`MENU-02` なら k=2）を求め、
   併置 `instruction.json` の `composition` から**同じ KEY の k 番目のエントリ**を引く。
   そのエントリに `type` があれば**それを使う**（表引きより優先・上の優先順位2）。
   無ければ表引きの結果に **`(k-1)` を足して `mod 6`**（インスタンス補正・§12.1.2(3.1)）。
...
| 2 | **`composition` の当該インスタンスの `type`**（KLK-088） | 併置 `instruction.json`（KEY＋出現順で引く） |
| 3 | 対象セクションの**現行マーカー**（`data-ref-id` がある参考準拠の案のみ） | 対象HTML（§12.2） |
| 4 | **表引き＋インスタンス補正**（§12.1.2(3.1)） | §12.1.2 / §12.1.3 |

`desiredType` があるときは 2〜4 を**見ない**。容器のマーカーを指定の型**ひとつだけ**に入れ替え
...
**★後段検証は「型が変わったか」だけでは足りない（KLK-080）:** 型マーカーだけを見ていると、
地図のアタリが `16/7` でも masonry の最終行に空白があっても、ブリッジは
「GALLERY-01 を pat-masonry にしました」と満足そうに報告してしまう。
**KLK-072〜076 で4回続けて起きたのは、まさにその手の違反だった。**

そこでブリッジは、再生成のあと**機械検査できる規約**を対象セクションについて見る
（`find_quality_warnings`）。**型指定の有無にかかわらず**実行する。

| 見るもの | 節 |
|---|---|
| 極端な横長比率（`16/7` など。許すのは 4/3・1/1・3/2 と型の例外だけ） | §3.0 |
| `min-height` だけで高さを決めたアタリ（HERO の全面ビジュアルは除く） | §3.0 |
| masonry / mosaic の**大小混在**と**最終行の充填**（`:nth-child(N)` 指定も読む） | §12.1.3 |
| 2カラム/3カラムでの**画像と本文の横並び**（カード2枚並べや番号バッジは対象外） | §8.1 |
| 型マーカーが1セクションに2つ以上付いていないか | §14 |

結果は `/status` の **`warnings`** に載る。空でなければ、**型が変わっていても成功と同じ顔をさせない**。
**判定できないものは黙る**（fail-open）。警告だらけにすると誰も読まなくなるので、
**確実に違反と言えるものだけ**を出す。

> 検査の範囲は「そのセクションのブロックで実際に使われているクラス名」で絞る。
> 容器名（`.m-access`）と型マーカー（`map-side`）だけでは足りない
> ——地図のアタリは `.map-atari` で、そのどちらも含まない（実装時に取りこぼした）。
