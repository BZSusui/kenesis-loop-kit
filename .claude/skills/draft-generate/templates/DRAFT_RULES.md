...
| 5 | `panel-band`（KLK-040新・KLK-041調整） | 全幅背景ビジュアル＋見出し（上〜中）＋**下部に横一列のフィルム風パネル群**（`display:grid;grid-template-columns:repeat(6,1fr);gap:4px;width:100%` の細かい多めコマ・`aspect-ratio:3/4`・`border-radius:0` で直角＝フィルムのコマ風・縦幅広め `min-height:480px`・**パネルは全幅中央配置（`grid-template-columns:1fr`・右寄せしない）**）。cat-0007/0019 の形。**整列＝`flex-end` / `center` / `center`**。モバイルはパネル3列 |
