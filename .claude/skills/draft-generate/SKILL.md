...
- **HERO/GALLERY/ABOUT/MENU の内部型プール（KLK-036/037/040/044・DRAFT_RULES §12.1.3）**: これらが `sections`（HERO=MV-01は常設）にあるときは、案ごとに
  **各セクションのプールから型を選ぶ**（archetype とは別軸・型数はセクション別）。選択は**算術せず表を読むだけ**: ① `data-columns`（正規化後）と
  `navPosition` を確定 → ② §12.1.2 の**オフセット表**（共有）で offset(0〜5) → ③ **§12.1.3 の該当セクション割り当て表**（GALLERY・MENU=mod4／HERO・ABOUT=mod6・offset 行→ (idxA,idxB,idxC)）
  ...
  **MENU（4型・KLK-044）**=`pat-cards`/`pat-list`/`pat-zigzag`/`price-table`（`.m-menu`・price-table=価格表/料金プラン型・表形式 grid）。overlap/img-overlap=重なり型・center-scroll/panel-band=大型MV・img-circle=円形・img-zigzag=左右交互複数段。
