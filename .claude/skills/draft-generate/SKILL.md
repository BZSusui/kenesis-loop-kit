...
- **HERO/GALLERY/ABOUT/MENU/SNS/NEWS/PRICE/FAQ/ACCESS/CONTACT の内部型プール（KLK-036/037/040/044/049/051/052/053/054/055・DRAFT_RULES §12.1.3）**: これらが `sections`（HERO=MV-01は常設）にあるときは、案ごとに
  **各セクションのプールから型を選ぶ**（archetype とは別軸・型数はセクション別）。選択は**算術せず表を読むだけ**: ① `data-columns`（正規化後）と
  `navPosition` を確定 → ② §12.1.2 の**オフセット表**（共有）で offset(0〜5) → ③ **§12.1.3 の該当セクション割り当て表**（GALLERY/MENU/HERO/ABOUT/SNS/NEWS/PRICE/FAQ/ACCESS/CONTACT すべて6型mod6・offset 行→ (idxA,idxB,idxC)）
