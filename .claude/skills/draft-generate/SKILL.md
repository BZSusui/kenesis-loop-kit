...
- **HERO/GALLERY/ABOUT/MENU/SNS/NEWS/PRICE/FAQ/ACCESS/CONTACT/SEARCH の内部型プール（KLK-036/037/040/044/049/051/052/053/054/055/056・DRAFT_RULES §12.1.3）**: これらが `sections`（HERO=MV-01は常設）にあるときは、案ごとに
  **各セクションのプールから型を選ぶ**（archetype とは別軸・型数はセクション別）。選択は**算術せず表を読むだけ**: ① `data-columns`（正規化後）と
  `navPosition` を確定 → ② §12.1.2 の**オフセット表**（共有）で offset(0〜5) → ③ **§12.1.3 の該当セクション割り当て表**（GALLERY/MENU/HERO/ABOUT/SNS/NEWS/PRICE/FAQ/ACCESS/CONTACT/SEARCH すべて6型mod6・offset 行→ (idxA,idxB,idxC)）
  ...
  **SNS（6型・mod6・KLK-049/050）**=`sns-grid`/`sns-slider`/`sns-cards`/`sns-masonry`/`sns-reels`/`sns-feed`（`.m-sns`・sns-grid=正方サムネ格子／sns-slider=横スクロールフィード／sns-cards=画像+キャプ+本文30字の横並び共通カード(VOICE同系)／sns-masonry=大小混在ベントー／sns-reels=縦長9:16リール帯／sns-feed=埋込風投稿カード縦列・実埋め込み禁止アタリ色面）、
  **NEWS（6型・mod6・KLK-051）**=`news-list`/`news-cards`/`news-media`/`news-timeline`/`news-table`/`news-accordion`（`.m-news`・news-list=日付+カテゴリバッジ+見出しの行リスト(従来default相当)／news-cards=サムネ+日付+見出し+抜粋の3列カード(PRICEプランカード流用)／news-media=画像左+テキスト右の横長メディア行縦積み／news-timeline=縦の時系列(日付+縦線+ドット)／news-table=日付|カテゴリ|見出しの表形式(PRICE料金表流用)／news-accordion=見出しクリックで開閉(FAQ Q&A流用)。「1カード/行を繰り返す」共通項で FAQ/PRICE へ流用可）、
  **PRICE（6型・mod6・KLK-052）**=`price-table`/`price-cards`/`price-featured`/`price-list`/`price-toggle`/`price-matrix`（`.m-price`・price-table=項目｜内容｜料金の表形式(現行default相当・news-table/MENU価格表流用)／price-cards=プラン比較3列カード(名称+価格+項目+申込・news-cards/pat-cards流用)／price-featured=中央プランをおすすめバッジ付きで一段大きく強調(feature-large流用)／price-list=「項目…¥価格」の縦リスト(news-list/pat-list流用)／price-toggle=月額/年額・A/Bをタブ切替+料金パネル(tab-switch流用・クリック動作)／price-matrix=プラン×機能の比較マトリクス◯×表(新規)。全型アタリ色面+仮文言）、
  **FAQ（6型・mod6・KLK-053）**=`faq-list`/`faq-accordion`/`faq-two-col`/`faq-cards`/`faq-category-tabs`/`faq-search`（`.m-faq`・faq-list=Qバッジ+質問/A+回答を開いた状態で縦積み(現行default相当・news-list流用)／faq-accordion=質問クリックで開閉(＋/−・details・news-accordion直接流用・現代FAQ定番)／faq-two-col=開閉Q&Aを2カラム(質問多数向け)／faq-cards=アイコン+質問+短答の3列カード(カテゴリ入口・news-cards/pat-cards流用)／faq-category-tabs=カテゴリタブ切替+Q&A(tab-switch/price-toggle流用・クリック動作)／faq-search=検索ボックス静的アタリ(飾り・送信なし)+アコーディオン(ヘルプセンター型)。全型アタリ色面+仮文言・外部依存ゼロ）、
  **ACCESS（6型・mod6・KLK-054）**=`map-side`/`map-top`/`map-overlay`/`map-hours`/`map-cards`/`map-steps`（`.m-access`・**全型に地図アタリ`.map-atari`を内包**[イラスト地図/Google Map 埋め込みを想定したプレースホルダ・実地図/実埋め込みなし・NFR-005]・map-side=地図横+住所/営業時間の2カラム(現行default相当・ABOUT img-left流用)／map-top=横長地図を上+下に情報／map-overlay=全幅地図に情報カードを重ねる(overlap流用・モダン)／map-hours=地図+営業/診療時間テーブル曜日×時間(price-table/news-table流用)／map-cards=複数店舗を小地図付きカードで並べる(news-cards/pat-cards流用・多店舗)／map-steps=地図+駅からの道順ステップ①→②→③(flow-row/flow-timeline流用・道案内)。全型仮文言+外部依存ゼロ）、
  **CONTACT（6型・mod6・KLK-055）**=`contact-cta`/`contact-form`/`contact-split`/`contact-methods`/`contact-banner`/`contact-steps`（`.m-contact`・**フォームは静的アタリ`.c-field`**[飾り・実送信なし・`<form action>`/外部URL/iframe なし・NFR-005]・contact-cta=見出し+ひとこと+ボタン(フォームなし誘導・現行default相当)／contact-form=お名前/メール/電話/内容の縦積みフォーム+送信ボタン／contact-split=左に連絡先情報+右にフォームの2カラム(ABOUT img-left流用)／contact-methods=電話/メール/LINE/来店の連絡手段を3〜4カード(news-cards/pat-cards流用)／contact-banner=全幅の色帯に大見出し+電話番号+ボタン(band/overlay流用・電話重視)／contact-steps=お問い合わせ→ご返信→ご相談の流れステップ+フォーム/ボタン(flow-row/flow-timeline流用)。全型仮文言+外部依存ゼロ）、
  **SEARCH（6型・mod6・KLK-056）**=`search-bar`/`search-keywords`/`search-filters`/`search-sidebar`/`search-header`/`search-hero`（`.m-search`・**入力欄は静的アタリ**[飾り・実送信なし・`<form action>`/外部URL/iframe なし・NFR-005]・①コンテンツ展開型=search-bar[中央の大検索バー・現行default相当]／search-keywords[キーワードをボタン化しパネル状に並べるムード系チップ群]／search-filters[カテゴリ/エリア/価格の絞り込みフォーム横並び・各項目は`<details>`クリック展開ドロップダウン]／search-sidebar[左に絞り込みナビ+右に結果カードグリッド2カラム・ABOUT grid+news-cards流用・cat-0011]、②小型検索窓(配置別)=search-header[ヘッダー(グローバルナビ)内の小型窓・ロゴ+メニュー+検索窓をカートボタン左隣に配置]／search-hero[メインビジュアル内に検索窓・背景アタリ+中央キャッチ+大検索バー+人気キーワード・旅行/不動産/求人/EC定番]。小型窓は入力+ボタン固定で配置idiomで型分け。本番制作時に実 header/HERO へ移す想定）。overlap/img-overlap=重なり型・center-scroll/panel-band=大型MV・img-circle=円形・img-zigzag=左右交互複数段。
  **offset0（1col×top）は各プール (index0,1,2)＝従来の archetype 既定と一致**（後方互換）。同一指示書＝同一割り当て・3案 distinct（連続3窓 mod N）。未選択は no-op。単案は idxA。
- **CTA マルチボタンと自動整列（KLK-058・DRAFT_RULES §4.4）**: `sectionOptions.CTA.buttons`（1〜4個・各 `{label, purpose?/href?}`）があれば複数ボタンを出す（無ければ従来の単一 `purpose`/`label`）。ボタン群 `<div class="cta-btns {marker}">`＋各 `<a class="cta-btn" href="#">` を出力し、**各 label の文字数で整列を決定**（single=1個中央／row=横並び[2個・3個短く同程度・4個短い]／stack=2個で両方長文の縦2段／two-plus-one=3個で1つだけ明らかに長文[その長文に `wide`＝`grid-column:1/-1` で全幅]／grid2=4個既定の2×2）。「長文」＝他の最長の約1.4倍以上（補助: 全角12字以上=長/8字以下=短）。CSS grid・静的アタリ（href 相対/#・実送信/外部URL なし・NFR-005）。モバイルは縦積み。
- **HERO / NAV 埋め込み検索窓（KLK-057・SEARCH 連動・DRAFT_RULES §12.1.3(7)）**: `sections` に **SEARCH が含まれる** とき、検索窓を重複させずに1箇所へ実埋め込みする（配置優先）: ①HERO 型が **`full`/`center-scroll`/`overlap`/`panel-band`（対応4型）** → `.m-hero` 内に `.hero-search`（`.hs-input`＋`.hs-btn`）を出力し `data-hero-search="on"`。②HERO 非対応（`split`/`band`）→ **NAV-01（バー型 `.m-nav`）の CTA 左隣に `.nav-search`（`.ns-input`＋`.ns-btn`）** を出力し `data-nav-search="on"`。いずれも**独立 SEARCH-01 セクションは出さない（一本化）**。**SEARCH 未選択なら `.hero-search`・`.nav-search` とも非生成**。検索窓は静的アタリ（実送信/`<form action>`/外部URL/iframe なし・NFR-005）。**「非生成」「フォールバック」等の仕組み説明はキャプションに出さない**（本文は自然な文言のみ）。
...
- **本文セクション選択（KLK-022・DRAFT_RULES §2.1）**: `NAV-01`/`MV-01`/`FOOTER-01` は**常時必須**。本文は
  ...
  **`layout.navPosition`** が `top`（既定）なら NAV-01 を MV-01 の上、`below-hero` なら下に置く。**SNS/地図は外部依存ゼロの
  ...
  （中身を揃え、配色・レイアウト型だけ振る）。
- **★ページ構成 `composition`（KLK-088・DRAFT_RULES §2.2）**: `instruction.composition` が**あるときは、それが本文構成の正**。
  - **配列の順＝ページの並び**。`sections` の canonical 順ではなく**この順で出す**。並びは**3案とも同じ**にする。
  - **同じ KEY が複数あるときは出現順に `-01` `-02` `-03`**（§2.0.1）。番地はページ内で一意にする
    （重複すると 🔄 部分再生成が止まる）。
  - 各エントリの **`heading` / `lead`**（§4.2）・**`moreLink`**（§4.3）は**そのインスタンスにだけ**効かせる。
    `sectionOptions.{KEY}` にも同じ文言が載っていることがあるが、**エントリ側を優先**する。
  - 各エントリの **`type`** があれば、そのセクションの型は**全案でその型**にする（表引きも参考準拠も見ない）。
  - `type` の無い2つ目以降は、表引き結果に **`(k-1)` を足して `mod 6`**（インスタンス補正・§12.1.2(3.1)）。
    **同じページに同じ型を2つ並べない**ため。
  - `sectionOptions.CTA.purpose/label` は**全 CTA インスタンス共通**（ボタン構成は共有・見出しだけ変えてよい）。
  - **`composition` が無い指示書は従来どおり**（`sections` を canonical 順）。解釈を変えない。
  - **受付チェック**: `sections` が `composition` のキー集合（canonical 順・重複除去）と食い違う指示書は、
    **生成を始めずに停止**して食い違いを報告する（曖昧なまま長い生成を走らせない）。
...
- **狭い本文カラムでの畳み方（§8.1・KLK-073）**: `layout.columns` が `2col-*` / `3col` のときは本文カラムが狭い。**カード内で画像＋本文を横並びにしない**（縦積み・画像が上）。セクションの列数も減らす（3列→2列、1fr 1fr→1fr）。画像の比率は 4/3 のまま。
- **文言の改行（§4.1.1・KLK-074）**: `copy` 指定が無くAIが文言を書く場合も、**改行位置はAIが決めて `<br>` を置く**（句点で改行が基本・キャッチは最大2行）。ブラウザの自動折り返し任せにしない。器の幅を内容にあわせ、`<br>` と幅不足の折り返しが二重にかからないようにする。
- **スクロール誘導（§4.3.1・KLK-074）**: `SCROLL ↓` は**クリックできること**。同一ページ内アンカー（`<a href="#…">`・JS不要）で、押したら**次に現れる塊が画面最上部に来る**ようにする（`2col-*` では `.m-layout` の先頭）。飾りテキストにしない。
- **スクロール誘導は画面端に縦組み（§4.3.2・KLK-097・重要）**: `SCROLL ↓` を**中央下に絶対配置しない**（`left:50%` / `translateX(-50%)` 禁止）。MV は `justify-content:center` の縦積みなので、中央下に置くと中身（キャッチ＋リード＋ボタン）が増えた時点で**ボタンと必ず重なる**（見本「サンプル和菓子店」案A で発生・KLK-097）。**左端に `writing-mode:vertical-rl` で縦組み**（右下は `.atari-tag` が占有）。矢印だけ `horizontal-tb`。**MV の左右 padding を 64px 以上・左右対称**にして誘導の帯を予約し、本文が入り込まないようにする。
- **MV はファーストビューを覆う（§4.3.3・KLK-097）**: `full`/`split`/`band`/`center-scroll` の縦幅は `min-height:520px;` → `calc(100vh - 64px);` → `max(520px, calc(100svh - 64px));` の**3行重ね**（`64px` はナビの実高・ナビが HERO の下なら差し引かない）。`@media print` で `320px` に戻す。`overlap`/`panel-band` は固有比率なので対象外。**構造の正 `SCR-002-compare.html` の `min-height:340px` は縮小プレビューの値なので写さないこと**（これを写して浅い MV が生成され続けた）。
- **横断ルールは型定義より優先する（KLK-075・重要）**: §3.0（比率 4/3）と §8.1（狭カラムは縦積み）は、型プール（§12.1.x）の説明文より**優先する**。型の説明に「横長」「横帯ワイド」「左右交互」とあっても、**比率は 4/3・`2col-*`/`3col` では縦積み**にする。実際に、規約追加後も型の語に引きずられて `aspect-ratio:16/7` の地図や、2カラムで横並びのままの `voice-zigzag` が生成された。**迷ったら横断ルールに従う。**
