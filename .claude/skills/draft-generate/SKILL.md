...
- **HERO/GALLERY/ABOUT/MENU/SNS/NEWS/PRICE/FAQ/ACCESS の内部型プール（KLK-036/037/040/044/049/051/052/053/054・DRAFT_RULES §12.1.3）**: これらが `sections`（HERO=MV-01は常設）にあるときは、案ごとに
  **各セクションのプールから型を選ぶ**（archetype とは別軸・型数はセクション別）。選択は**算術せず表を読むだけ**: ① `data-columns`（正規化後）と
  `navPosition` を確定 → ② §12.1.2 の**オフセット表**（共有）で offset(0〜5) → ③ **§12.1.3 の該当セクション割り当て表**（GALLERY/MENU/HERO/ABOUT/SNS/NEWS/PRICE/FAQ/ACCESS すべて6型mod6・offset 行→ (idxA,idxB,idxC)）
  ...
  **ACCESS（6型・mod6・KLK-054）**=`map-side`/`map-top`/`map-overlay`/`map-hours`/`map-cards`/`map-steps`（`.m-access`・**全型に地図アタリ`.map-atari`を内包**[イラスト地図/Google Map 埋め込みを想定したプレースホルダ・実地図/実埋め込みなし・NFR-005]・map-side=地図横+住所/営業時間の2カラム(現行default相当・ABOUT img-left流用)／map-top=横長地図を上+下に情報／map-overlay=全幅地図に情報カードを重ねる(overlap流用・モダン)／map-hours=地図+営業/診療時間テーブル曜日×時間(price-table/news-table流用)／map-cards=複数店舗を小地図付きカードで並べる(news-cards/pat-cards流用・多店舗)／map-steps=地図+駅からの道順ステップ①→②→③(flow-row/flow-timeline流用・道案内)。全型仮文言+外部依存ゼロ）。overlap/img-overlap=重なり型・center-scroll/panel-band=大型MV・img-circle=円形・img-zigzag=左右交互複数段。
