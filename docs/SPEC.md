...
| REQ-005 | デザインラフをHTMLで生成できる(コンテンツ配置・アタリ画像・グローバルナビ位置・配色適用・スクロール出現/読込アニメーション) | SCR-002 | 生成失敗時も生成指示書は保持し、再実行できる。アニメーションは生成指示書 `output.animation`(既定ON)で切替。OFFの場合は `.reveal`/`IntersectionObserver` を出力せず即時全表示(REQ-002拡張)。**2026-09-05・KLK-086〜089: ページ構成 `composition` を追加**（順序つき・重複あり・エントリ個別設定つきの本文構成）。同じセクションを複数置け（本文10種＋CTA は各3個・ACCESS/CONTACT/SEARCH は1個・本文合計12個）、並び順を指定でき（**指定時は3案とも同じ並び**）、エントリごとに見出し・リード文・詳細ページ誘導ボタン・**レイアウト型**を指定できる（型を明示した番地は3案とも同じ型）。番地は出現順に `{KEY}-01`/`-02`/`-03`。**`composition` の無い指示書は従来と1バイトも解釈が変わらない**（additive）。規約は DRAFT_RULES §2.0.1／§2.2／§12.1.2(3.1)／§12.1／§12.2／§14 |
...
| REQ-008 | 複数案(最大3案)を一括生成し、サムネイル一覧・ボタンでの案切替・クリックで原寸を別タブ表示して比較できる | SCR-002 | 一部の案のみ失敗した場合は成功分を表示し、失敗を通知。**2026-09-05・KLK-092: `variants:1` でも `compare.html` を生成する**。案切替のセグメント・サムネイル列は出さないが、**表示幅の切替と 🔄 セクション再生成は3案と同等**に使える（それらは compare.html の上に載っているため、作らないと機能が丸ごと失われる）。🔄 が送る `letter` は空文字で `index.html` を指す |
