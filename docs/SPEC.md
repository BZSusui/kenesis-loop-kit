...
| 番地ラベル | 生成モックの各セクションに振るID表示(例: HERO-01)。部分再生成の指示に使う。印刷時は非表示 |
...
| REQ-010 | 生成物と生成指示書は `mockups/{日付}_{案件名}/` に自動整理し、完了後に保存先フォルダを自動で開く | - | フォルダが開けない場合は保存先パスを表示 |
...
| ID | 要件 | 関連画面 | 備考 |
|---|---|---|---|
| REQ-101 | デザインテイストを単一選択チップで指定できる | SCR-001 | OQ-002(語彙一覧)に依存 |
| REQ-102 | 見本サイトのURLを入力し、配色・構成の傾向を生成に反映できる | SCR-001 | 読み取れないURLはその旨を通知しURLなしで続行 |
| REQ-103 | 番地ラベル(例: HERO-01)を指定してセクション単位で再生成できる | SCR-002 | |
| REQ-104 | アタリ画像にフリー実写真を使うオプション(b方式) | SCR-001 | 通信必須。イメージ違いは許容 |
...
| ID | 要件 | 関連画面 | 備考 |
|---|---|---|---|
| REQ-201 | スマホ表示版の同時生成(ブラウザ幅の数値指定対応) | SCR-001, SCR-002 | |
| REQ-202 | 生成履歴の保存・見返し | SCR-003 | |
...
| SCR-003 | 履歴一覧画面(Could) | 過去の生成の見返し | 日付・業種・設定要約・再表示リンク | REQ-202 | 生成ユーザー | docs/wireframes/SCR-003-history.html |
...
flowchart LR
    ...
    SCR003[SCR-003 履歴一覧] -->|再表示| SCR002
```
...
- スマートフォンアプリ(Webブラウザのみ。REQ-201はスマホ「表示の生成」であり別物)
...
| OQ-002 | テイスト選択肢の語彙一覧(muuuuu.orgの分類を参考にユーザーが確定) | 未決定 | |
| OQ-003 | 業種一覧の初期リスト(実績100点のタグ付け結果から起こしてユーザーが確定) | 未決定 | |
